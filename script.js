...
let resources = JSON.parse(localStorage.getItem("mineClickerSave"));
...
function renderShop() {
  ...
  autoGenBtn.textContent = "Comprar Gerador Automático (10 moedas)";
  autoGenBtn.onclick = () => {
    if (moedas >= 10) {
      moedas -= 10;
      setInterval(autoGenerate, 1000);
      autoGenBtn.disabled = true;
      autoGenBtn.textContent = "Gerador Ativado";
      renderShop();
    }
  };
  shopDiv.appendChild(autoGenBtn);
